...
if (obj_eld_controller.game_over)
{
	...
	if (obj_eld_controller.game_won)
		draw_text_center(_rw2, _rh2-35, "YOU WON", 2, 2, 0, lerp(c_yellow, c_white, sin(_ct)*0.5 + 0.5), 1.0);
	else
		draw_text_center(_rw2, _rh2-35, "YOU DIED", 2, 2, 0, c_red, 1.0);
	
	draw_text_center(_rw2, _rh2, "Final Score", 1, 1, 0, c_white, 1.0);
	draw_set_halign(fa_right);
	draw_text(_rw2+42, _rh2+15, string(_sc));
	draw_text(_rw2+42, _rh2+40, "+ "+string(extra_lives) + " x " + string(_sc));
	draw_set_color(c_yellow);
	draw_text(_rw2+42, _rh2+65, "= "+string(_sc * (extra_lives + 1)));
...
// FUEL RESERVES
//////////////////
draw_text_transformed(1,3, "Fuel ", 0.5, 0.5, 0);
...
draw_text_transformed(room_width - 25, 3, string(_lives) + "x", 0.5, 0.5, 0);
